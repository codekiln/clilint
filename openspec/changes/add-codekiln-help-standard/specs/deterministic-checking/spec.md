...
behavior, and a bounded timeout. A bundle-owned checker SHALL own its tested
...
standard error. A bundle-owned checker SHALL return the evidence required by
the shared Check Result contract.
...
check unless its checker explicitly supplies input. A bundle-owned checker
SHALL control standard input for the tested CLI tool invocations it performs.
